...
| 文件存储 | Testcontainers MinIO（S3 兼容，通过 Docker 自动启动） |
...
在开发模式下，运行 `./gradlew server-admin:bootRun` 时，Testcontainers 会自动启动 PostgreSQL、Redis 和 MinIO 的 Docker 容器。无需手动配置。
